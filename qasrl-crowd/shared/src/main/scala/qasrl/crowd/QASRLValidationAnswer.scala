...
sealed trait QASRLValidationAnswer {
  ...
  def isRedundant : Boolean = this match {
    case RedundantQuestion => true
    case _ => false
  }

  def getAnswer: Option[Answer] = this match {
    case a@Answer(_) => Some(a)
    case _ => None
  }

  def isAnswer : Boolean = getAnswer.nonEmpty
  ...
  def getSpans : List[Span] = this match {
    case Answer(spans) => spans
    case _ => Nil
  }

  def agreesWith(that: QASRLValidationAnswer): Boolean = (this, that) match {
...
object QASRLValidationAnswer {
  ...
  def numValidQuestions(responses: List[List[QASRLValidationAnswer]]) =
    math.round(responses.map(_.count(!_.isInvalid)).meanOpt.get - 0.01).toInt
